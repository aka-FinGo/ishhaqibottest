...
// inline stillar → components.css klasslari
// ============================================================
...
function renderWorkflowSteps() {
    ...
    const topControls = `
        <div style="background:var(--surface); border:1px solid var(--border); border-radius:18px; padding:18px; margin-bottom:20px; box-shadow:var(--shadow-sm);">
            <div style="margin-bottom:15px;">
                ...
                <div style="display:flex; gap:10px;">
                    ...
                    <button onclick="saveStrictSettingUI()" class="btn-main"
                        style="flex:1; margin-top:0; height:46px; font-size:13px; background:var(--blue);">
                        Saqlash
                    </button>
                </div>
            ...
            <button id="stageConfigBtn" class="btn-secondary"
                style="width:100%; height:44px; border-color:var(--blue); color:var(--blue); border-radius:12px; font-weight:700; background:rgba(59,130,246,0.05);">
    ...
    currentWorkflowSteps.forEach((step, idx) => {
        ...
        html += `
        <div class="card" style="margin-bottom:20px; border:2px solid ${phaseColors.bg}; background:var(--surface); padding:16px; border-radius:18px; box-shadow:var(--shadow-md);">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; padding-bottom:10px; border-bottom:1px solid var(--border);">
                ...
                <div style="display:flex; gap:6px;">
                    <button class="del-icon-btn move-up-btn"   data-idx="${idx}" style="background:var(--bg); color:var(--text-muted); width:30px; height:30px; font-size:12px;" ${idx === 0 ? 'disabled' : ''}>▲</button>
                    <button class="del-icon-btn move-down-btn" data-idx="${idx}" style="background:var(--bg); color:var(--text-muted); width:30px; height:30px; font-size:12px;" ${idx === currentWorkflowSteps.length - 1 ? 'disabled' : ''}>▼</button>
                    <button class="del-icon-btn delete-step-btn" data-idx="${idx}" style="background:var(--comp-del-bg); color:var(--comp-del-text); width:30px; height:30px; font-size:12px;">🗑</button>
                </div>
            </div>
            ...
            <div class="filter-row" style="margin-bottom:12px; display:flex; gap:10px;">
                ...
                <div style="flex:1;">
                    ...
                    <input type="text" class="action-input" data-idx="${idx}"
                        value="${escapeHtml(step.action)}" placeholder="Masalan: Men kesdim"
            ...
            <div style="margin-bottom:4px;">
                ...
                <input type="text" class="status-input" data-idx="${idx}"
                    value="${escapeHtml(step.status)}" placeholder="Masalan: Kesildi"
                    style="border-radius:12px; font-weight:600;">
            </div>
        </div>`;
    });
...
function addNewWorkflowStep() {
    currentWorkflowSteps.push({ position: '', action: 'Bajardim', status: 'Bajarildi', isStart: false });
    renderWorkflowSteps();
}
...
async function saveWorkflowConfigUI() {
    if (!currentWorkflowSteps.length) {
        showToastMsg('❌ Kamida bitta bosqich bo\'lishi shart', true); return;
    }
...
/* ---------------------------------------------------------------
   showStageConfigDialog — inline stillar → CSS klasslari
   --------------------------------------------------------------- */
function showStageConfigDialog() {
    const dialog = document.createElement('div');
    dialog.id = 'stageConfigDialog';
    dialog.className = 'sc-overlay';

    dialog.innerHTML = `
        <div class="sc-box">
            <h3 class="sc-title">Boshlanish / Yakunlash bosqichi</h3>
            <p class="sc-desc">
                Qaysi jarayondan keyin boshlanish va yakunlash bosqichi tugashi kerakligini belgilang:
            </p>

            <div class="sc-group">
                <label class="sc-label">Boshlanish bosqichi:</label>
                <select id="startStageSelect" class="sc-select">
                    <option value="">Tanlanmagan</option>
                    ${currentWorkflowSteps.map((step, idx) =>
                        `<option value="${idx}" ${step.isStart ? 'selected' : ''}>
                            ${step.position || `Bosqich ${idx + 1}`}
                        </option>`
                    ).join('')}
                </select>
            </div>
            ...
            <div class="sc-group">
                <label class="sc-label">Yakunlash bosqichi:</label>
                <select id="endStageSelect" class="sc-select">
                    <option value="">Tanlanmagan</option>
                    ${currentWorkflowSteps.map((step, idx) =>
                        `<option value="${idx}" ${step.isEnd ? 'selected' : ''}>
                            ${step.position || `Bosqich ${idx + 1}`}
                        </option>`
                    ).join('')}
                </select>
            ...
            <div class="sc-actions">
                <button id="saveStageConfig"   class="sc-save">Saqlash</button>
                <button id="cancelStageConfig" class="sc-cancel-btn">Bekor qilish</button>
            </div>
        </div>
    `;
