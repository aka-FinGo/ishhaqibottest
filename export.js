async function exportToBot(dataList, fileNameBase, scope) {
    ...
    try {
        ...
        dataList.forEach((r, idx) => {
            ...
            ws["B" + row] = {
                v: r.name || (typeof employeeName !== "undefined" ? employeeName : ""),
                t: "s",
                s: dStyle,
            };
        ...
        const fileName =
            fileNameBase + "_" + new Date().toISOString().slice(0, 10) + ".xlsx";
        const result = await apiRequest(
            { action: "export_to_bot", base64, fileName, scope: scope || "self" },
            { timeoutMs: 30000 },
        );
        tg.MainButton.hide();
        if (result.success) {
...
async function exportAdminExcel() {
    ...
    const baseName =
        empVal !== "all" ? "Hisobot_" + empVal : "Kompaniya_Umumiy_Hisoboti";
    try {
