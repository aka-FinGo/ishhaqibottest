...
function loadCachedData() {
  try {
    ...
    try {
      localStorage.removeItem('myFullRecords');
    } catch (e2) {}
  }
  return false;
}

// ✅ FIX: Safe localStorage save
function saveCacheData(data) {
  try {
    ...
    if (e.name === 'QuotaExceededError') {
      console.warn('⚠️ localStorage quota to\'lgan');
    } else {
      console.error('❌ Cache save xatosi:', e);
    }
  }
}

// ✅ FIX: Initialize app with retry logic
async function initializeApp() {
  try {
    const firstName = user ? user.first_name : 'Xodim';
    document.getElementById('greeting').innerText = `Salom, ${firstName}!`;

    // Step 1: Load cached data (offline-first strategy)
    const cacheLoaded = loadCachedData();
    if (cacheLoaded && myFullRecords.length > 0) {
      if (typeof initMyFilters === 'function') {
        initMyFilters();
      }
    }
    ...
    if (data && data.success) {
      ...
      canExportCompanyData =
        myRole === 'SuperAdmin' ||
        (myPermissions.canViewAll && myPermissions.canExport);
      ...
      if (data.autoAdded) {
        showToastMsg(
          "✅ Siz ro'yxatga qo'shildingiz. Ruxsat uchun admin bilan bog'laning."
        );
      }
      ...
      console.log('✅ App initsializatsiya muvaffaqiyatli bajarildi');
      _appInitialized = true;
      _appInitRetries = 0;
    } else {
      throw new Error(data?.error || 'Init xatosi');
    }
  } catch (error) {
