...
/* ---------------------------------------------------------------
   ...
   --------------------------------------------------------------- */
// enhanced_admin_list.js — loadAdminData funksiyasini to'liq almashtiring
...
async function loadAdminData() {
    ...
    // 2. Admin lekin canViewAll yo'q → FAQAT O'Z yozuvlarini ko'rsatamiz
    const canViewAll = myRole === 'SuperAdmin' || (myPermissions && myPermissions.canViewAll);

    if (!canViewAll) {
        // O'z yozuvlarini myFullRecords dan yuklaymiz
        const ownData = (typeof myFullRecords !== 'undefined' && Array.isArray(myFullRecords))
            ? myFullRecords : [];

        globalAdminData = ownData;
        ...
        // Jami budjet maydonlariga o'z summalarini yozamiz
        let totalUZS = 0, totalUSD = 0, totalUZSConv = 0;
        ownData.forEach(item => {
            const uzs = Number(item.amountUZS) || 0;
            const usd = Number(item.amountUSD) || 0;
            totalUSD += usd;
            if (usd > 0) totalUZSConv += uzs;
            else totalUZS += uzs;
        });
        const effectiveTotal = totalUZS + totalUZSConv;
        ...
        if (budgetEl)  budgetEl.textContent  = effectiveTotal.toLocaleString() + ' UZS';

        // Export tugmasini yashirish (ruxsat yo'q)
    ...
    // 3. To'liq ruxsat bor (SuperAdmin, Direktor, canViewAll=true Admin)
    // Export tugmasini ko'rsatish
    const exportBtn = document.getElementById('companyExportBtn');
    if (exportBtn) {
        exportBtn.style.display = (myRole === 'SuperAdmin' || (myPermissions && myPermissions.canExport))
            ? '' : 'none';
    }
    ...
    try {
        ...
        if (response.success) {
            ...
            throw new Error(response.error || 'Ma\'lumotlarni yuklashda xatolik');
        }
    } catch (error) {
